...
def queryAPI(donorId, probeIds):
    ...
    if(url1 != url):
        ...
        return
    print(url)
    response = urllib.request.urlopen(url1).read().decode('utf8')
    text = json.loads(response)
    data = text['msg']
    samples = []
    probes = []
...
def getAPIData(donorIds, probeIds):
    apiData = []
    for i in range(0, len(donorIds)):
        res = queryAPI(donorIds[i], probeIds)
        apiData.append(res)
    return apiData;
